...
# Altbilgiye Dinamik Sayfa Numarası Ekleyen Fonksiyon
...
if st.button("Raporu Oluştur (.docx)"):
    ...
    # İçerik sayfalarına otomatik sayfa numarası (Footer) ekleme
    footer = cover_section.footer
    ...
    akiskan_maddeleri = [
        f"1. Kalorifer tesisatında {rej_kalorifer} °C sıcak su.",
        f"2. Fan-Coil ısıtma tesisatında {rej_fco_ist} °C sıcak su.",
        f"3. Fan-Coil Soğutma tesisatında {rej_fco_sog} °C soğuk su.",
        f"4. Klima santrali ısıtma tesisatında {rej_ks_ist} °C sıcak su.",
        f"5. Klima santrali Soğutma tesisatında {rej_ks_sog} °C soğuk su.",
        f"6. Boyler ısıtma tesisatında {rej_boyler} °C sıcak su.",
        f"7. Kullanma sıcak suyunda {rej_k_sicak} °C sıcak su.",
        f"8. Döşemeden ısıtma tesisatında {rej_doseme} °C sıcak su."
    ]

    if aktif_buhar:
        akiskan_maddeleri.append(f"9. Buhar tesisatında {rej_buhar} buhar")
    if aktif_kizgin:
        akiskan_maddeleri.append(f"10. Kızgın su tesisatında {rej_kizgin} °C sıcak su.")
    ...
    st.success("Altbilgi sayfa numaraları ve içindekiler yapısı güncellenerek Word dosyası hazırlandı!")
